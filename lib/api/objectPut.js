...
function _storeIt(bucketName, bucketMD, objectKey, objMD, authInfo, canonicalID,
    ...
    return async.waterfall([
        ...
        (infoArr, callback) => {
            ...
            return services.versioningPreprocessing(request, bucketMD,
                    bucketName, objMD, metadataStoreParams.objectKey, log,
                    callback);
        },
        (options, callback) => {
        ...
        if (err === null || err === undefined) {
            pushMetric('putObject', log, { authInfo, bucket: bucketName,
                newByteLength: size, oldByteLength: prevContentLen });
        }
...
function objectPut(authInfo, request, streamingV4Params, log, callback) {
    ...
    const versionId = request.query ? request.query.versionId : undefined;
    const valParams = { authInfo, bucketName, objectKey,
                        versionId, requestType, log };
    ...
    services.metadataValidateAuthorization(valParams, (err, bucket, objMD) => {
        ...
        return async.waterfall([
            ...
            (cipherBundle, callback) => _storeIt(bucketName, bucket, objectKey,
                objMD, authInfo, canonicalID, cipherBundle, request,
                streamingV4Params, log, callback),
        ], (err, res) => callback(err, res, corsHeaders));
    });
    return null;
}
