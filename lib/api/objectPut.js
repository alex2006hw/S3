...
function _storeIt(bucketName, objectKey, objMD, authInfo, canonicalID,
    ...
    return async.waterfall([
        ...
        (dataGetInfoArr, callback) => _storeInMDandDeleteData(bucketName,
                dataGetInfoArr, cipherBundle, metadataStoreParams,
                dataToDelete, requestLogger, callback),
    ], (err, contentMD5) => {
...
function objectPut(authInfo, request, streamingV4Params, log, callback) {
    ...
    services.metadataValidateAuthorization(valParams, (err, bucket, objMD) => {
        ...
        return async.waterfall([
            ...
            (cipherBundle, callback) => _storeIt(bucketName, objectKey,
                objMD, authInfo, canonicalID, cipherBundle, request,
                streamingV4Params, log, callback),
        ], (err, contentMD5) => callback(err, contentMD5, corsHeaders));
    });
    return null;
}
