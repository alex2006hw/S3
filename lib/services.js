...
const VID_INF = versioning.VersioningConstants.VID_INF;
...
export default {
    ...
    */
    ...
    metadataStoreObject(bucketName, dataGetInfo, cipherBundle, params, cb) {
        ...
        const options = { versioning, versionId };
        ...
        async.waterfall([
            ...
            const versionId = data ? JSON.parse(data).versionId : undefined;
    ...
    */
    ...
    versioningPreprocessing(request, bucketName, bucketMD, objectKey, objMD,
            log, callback) {
        const options = {};
        // bucket is not versioning enabled
        if (bucketMD.versioningConfiguration === undefined) {
            options.deleteData = true;
            return callback(null, options);
        }
        // bucket is versioning enabled
        const reqVersionId = request.query ?
            request.query.versionId : undefined;
        const mstVersionId = objMD ? objMD.versionId : undefined;
        const mstIsNull = objMD ? objMD.isNull : false;
        const vstat = bucketMD.versioningConfiguration.Status;
        if (!reqVersionId) {
            ...
            if (mstVersionId === undefined || mstIsNull) {
                ...
                // versioning is enabled, store master version in a new key
