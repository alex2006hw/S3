...
export default {
    ...
    metadataValidateAuthorization(params, cb) {
        const { authInfo, bucketName, objectKey, requestType, log, versionId }
            = params;
        const canonicalID = authInfo.getCanonicalID();
        const versioningParams = versionId ? { versionId } : {};
    ...
    */
    ...
    metadataStoreObject(bucketName, dataGetInfo, cipherBundle, params, cb) {
        const { objectKey, authInfo, size, contentMD5, metaHeaders,
            ...
            lastModifiedDate, versioningParams, isNull } = params;
        ...
        // store whether current version is a null version in metadata
        omVal.isNull = isNull;
        omVal.acl = {
            Canned: 'private',
            FULL_CONTROL: [],
        ...
        if (!multipart) {
            ...
            acl.parseAclFromHeaders(parseAclParams, (err, parsedACL) => {
                ...
                omVal.acl = parsedACL;
                metadata.putObjectMD(bucketName, objectKey, omVal,
                versioningParams, log, (err, versionId) => {
                    if (err) {
                        log.error('error from metadata', { error: err });
                        return cb(err);
                    }
    ...
    deleteObject(bucketName, objectMD, objectKey, log, cb) {
        ...
        if (objectMD['x-amz-version-id'] === undefined) {
