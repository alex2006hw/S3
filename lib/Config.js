...
class Config {
    constructor() {
         ...
         * By default, the locationConfig file is "locationConfig.json" at the
         * root. It can be overridden using the S3_LOCATION_FILE environment
         * var.
         */
    ...
    _getLocationConfig() {
        ...
        // check exist file
        try {
            fs.statSync(this.locationConfigPath);
        } catch (err) {
            if (err) {
                // location file does not exist
                // legacy config format where only one config file
                return;
            }
        }
        try {
                      ...
                      undefined) {
                        ...
                        if (fields.indexOf('bootstrap') > -1) {
                            this.locationConstraints[l].details
                                ...
                                locationConfig[l].details.connector.
                                sproxyd.bootstrap;
                        }
    ...
    _getConfig() {
        ...
        if (config.regions !== undefined) {
            assert(config.locationConstraints === undefined &&
                config.restEndpoints === undefined,
                'bad config: use either regions or locationConstraints/' +
                'restEndpoints configuration, not both');
            assert(typeof config.regions === 'object',
                'bad config: regions must be an object');
